...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_SIZE) {
      return NextResponse.json({ error: 'Plik za duży. Maksymalny rozmiar: 50MB' }, { status: 400 })
    }
    ...
    await supabase.storage.createBucket('downloads', { public: true }).catch(() => {})
    ...
    const { data: urlData } = supabase.storage.from('downloads').getPublicUrl(pathname)
